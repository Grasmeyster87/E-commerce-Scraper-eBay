...
export default function Sidebar({
    ...
    return (
        <div className="w-80 bg-slate-950 border-r border-slate-800 p-4 flex flex-col gap-4 overflow-y-auto h-screen select-none custom-scrollbar">
            {/* Header / Brand Logo */}
            <div className="flex items-center gap-3 px-2 py-1">
                ...
                <div>
                    <h1 className="text-xs font-mono font-bold text-slate-200 tracking-wider">EBAY SCRAPER</h1>
                    <p className="text-[10px] font-mono text-indigo-400">v1.2.0 • Enterprise Edition</p>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-3 flex-1">
                ...
                <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-900/30 transition-all">
                    ...
                    >
                        <span className="flex items-center gap-2">🚀 AUTOMATION CONTROLLER</span>
                        <span className={`text-slate-500 transition-transform duration-200 ${openSections.automation ? 'rotate-180' : ''}`}>
                            ▼
                        </span>
                    </button>
                    ...
                    {openSections.automation && (
                        <div className="p-4 bg-slate-950/40 border-t border-slate-900 space-y-4 animate-fadeIn">
                            {/* Target Items Limit */}
                            <div>
                                <label className="block text-[10px] text-slate-500 font-mono mb-1">ITEMS PER PAGE REQUEST:</label>
                                <select
                                    value={itemsPerPageSelection}
                                    onChange={(e) => setItemsPerPageSelection(Number(e.target.value))}
                                ...
                                >
                                    <option value={60}>60 Items (Standard)</option>
                                    <option value={120}>120 Items (Extended)</option>
                                    <option value={240}>240 Items (Max Batch)</option>
                                </select>
                            </div>
                            ...
                            <div className="bg-slate-950/60 border border-slate-900 p-3 rounded-xl flex items-center justify-between">
                                <div>
                                    <p className="text-xs font-mono text-slate-300">Scrape All Pages</p>
                                    <p className="text-[9px] font-mono text-slate-600">Recursive execution loop</p>
                                </div>
                                <input
                                    type="checkbox"
                            ...
                            {!scrapeAllPages && (
                                <div className="animate-slideDown">
                                    <label className="block text-[10px] text-slate-500 font-mono mb-1">MAX BOUNDED PAGES:</label>
                                    <input
                                        type="number"
                                        min={1}
                                        max={100}
                                        value={maxScrapePages}
                                        onChange={(e) => setMaxScrapePages(Math.max(1, parseInt(e.target.value) || 1))}
                            ...
                            <button
                                type="button"
                                onClick={onNextPage}
                                disabled={loading || (pagination && pagination.currentPage >= pagination.totalPages)}
                ...
                <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-900/30 transition-all">
                    <button
                        ...
                        className="w-full bg-slate-900/80 hover:bg-slate-900 px-4 py-3 text-left text-[11px] font-mono font-bold text-slate-300 flex items-center justify-between transition-colors cursor-pointer"
                    >
                        <span className="flex items-center gap-2">🗄️ STORAGE & DATASET TARGETS</span>
                        <span className={`text-slate-500 transition-transform duration-200 ${openSections.storage ? 'rotate-180' : ''}`}>
                            ▼
                        </span>
                    </button>
                    ...
                    {openSections.storage && (
                        <div className="p-4 bg-slate-950/40 border-t border-slate-900 space-y-4 animate-fadeIn">
                            {/* Directory Config */}
                            <div>
                                <label className="block text-[10px] text-slate-500 font-mono mb-1">EXPORT DIRECTORY PATH:</label>
                                <input
                                    type="text"
                                    value={saveDir}
                            ...
                            <div className="space-y-3 p-3 bg-slate-950/50 border border-slate-900 rounded-xl">
                                <label className="block text-[10px] text-cyan-500 font-mono font-bold">SQLITE3 CONFIGURATION:</label>
                            ...
                            <div>
                                <label className="block text-[10px] text-slate-500 font-mono mb-1">ACTIVE TABLE (DATASET):</label>
                                <select
                                    value={activeTable || ""}
                ...
                <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-900/30 transition-all">
                    <button
                        ...
                        className="w-full bg-slate-900/80 hover:bg-slate-900 px-4 py-3 text-left text-[11px] font-mono font-bold text-slate-300 flex items-center justify-between transition-colors cursor-pointer"
                    >
                        <span className="flex items-center gap-2">🛠️ WIZARDS & I/O OPERATIONS</span>
                        <span className={`text-slate-500 transition-transform duration-200 ${openSections.exports ? 'rotate-180' : ''}`}>
                            ▼
                        </span>
                    </button>
                    ...
                    {openSections.exports && (
                        <div className="p-4 bg-slate-950/40 border-t border-slate-900 space-y-4 animate-fadeIn">
                            ...
                            <div className="space-y-2">
                                <label className="block text-[10px] text-slate-500 font-mono mb-1">SERIALIZE SINGLE EXPORT:</label>
                                <div className="grid grid-cols-2 gap-2">
                                    {['csv', 'json', 'xml', 'pdf'].map((fmt) => (
                                        <button
                                            key={fmt}
                                            type="button"
            ...
            {pagination && (
                <div className="bg-slate-950 border border-slate-900 p-3 rounded-xl font-mono text-[10px] text-slate-500 space-y-1 mt-auto">
                    <div className="flex justify-between">
                        ...
                        <span className={loading ? "text-amber-500 animate-pulse" : "text-emerald-500"}>
                            {loading ? "PROCESSING" : "IDLE"}
                        </span>
                    </div>
                    <div className="flex justify-between">
                        <span>PAGINATION NODE:</span>
                        <span className="text-slate-300">Page {pagination.currentPage} / {pagination.totalPages || '?'}</span>
                    </div>
                    <div className="flex justify-between">
                        <span>BUFFER SIZE:</span>
                        <span className="text-slate-300">{results?.length || 0} Entities</span>
                    </div>
                </div>
            )}
        </div>
